...
Updated: Robust Driver Photo Loading (Checks PNG/JPG/AVIF)
"""
...
class DriverInfoWidget(QFrame):
    ...
    def __init__(self, name, number, assets_path):
        super().__init__()
        ...
        # Try to find a valid photo file
        photo_path = self.find_driver_photo(assets_path)
        self.setup_ui(name, number, photo_path)

    def find_driver_photo(self, assets_path):
        """Check for various image formats"""
        # Priority order: PNG -> JPG -> JPEG -> AVIF
        filenames = ['driver_pic.png', 'driver_pic.jpg', 'driver_pic.jpeg', 'driver_pic.avif']
        
        for fname in filenames:
            path = os.path.join(assets_path, fname)
            if os.path.exists(path):
                print(f"✅ Found driver photo: {fname}")
                return path
        
        print("⚠️ No driver photo found (checked png, jpg, jpeg, avif)")
        return None
    ...
    def setup_ui(self, name, number, photo_path):
        ...
        if photo_path:
            loaded = pixmap.load(photo_path)
            if not loaded:
                print(f"❌ Failed to load image data from {photo_path}")
        ...
        # Draw image scaled to cover the circle (aspect ratio preserved)
        painter.setBrush(QBrush(pixmap.scaled(
...
class AdsDisplayWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Note: We pass assets_path so it can find the photo automatically
